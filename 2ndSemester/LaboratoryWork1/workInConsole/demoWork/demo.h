...
void demon() {
    ...
    cout << MemoryDatabase::getTrainsWithIDFromMemory();
    cout << RedInConsole + "Printed with ID" + StandardConsole << endl << endl;

    cout << RedInConsole + "Deleting Train with ID = " + to_string(MemoryDatabase::getMaxIDInMemory() - 2) + " in memory ..." +
            StandardConsole << endl;
    MemoryDatabase::deleteTrainInMemory(MemoryDatabase::getMaxIDInMemory() - 2);
    cout << RedInConsole + "Deleted" + StandardConsole << endl << endl;
    ...
    cout << RedInConsole + "Changing Train name with ID = " + to_string(MemoryDatabase::getMaxIDInMemory() - 2) + " in memory ..." +
            StandardConsole << endl;
    randomTrain.optionalName = "normalName";
    MemoryDatabase::changeTrainInMemory(MemoryDatabase::getMaxIDInMemory() - 2, randomTrain);
    ...
    taskResults = MemoryDatabase::task1InMemory("ame");
    ...
    cout << "Train (ID = " << to_string(MemoryDatabase::getMaxIDInMemory()) << ") set Destination = \"FAST\" and number = \"6666\""
         << StandardConsole << endl;
    randomTrain.destination = FAST;
    randomTrain.number = "6666";
    MemoryDatabase::changeTrainInMemory(MemoryDatabase::getMaxIDInMemory(), randomTrain);
    ...
    taskResults = MemoryDatabase::task2InMemory(FAST, "6000", "7000");
    cout << RedInConsole + "Results:" + StandardConsole << endl;
    for (int i = 0; i < taskResults.size(); ++i) {
        cout << taskResults[i].getInText() << endl;
    }
    ...
    cout << "Train (ID = " << to_string(MemoryDatabase::getMaxIDInMemory()) << ") set Departure = \"20:50 20/6/2000\""
         << StandardConsole << endl;
    ...
    taskResults = MemoryDatabase::task3InMemory(testDate);
    cout << RedInConsole + "Results:" + StandardConsole << endl;
    for (int i = 0; i < taskResults.size(); ++i) {
        cout << taskResults[i].getInText() << endl;
    }
