...
void task1InMemory() {
    ...
    taskResult = task1InMemory(lastFragmentOfName);
...
void task2InMemory() {
    ...
    if (stoi(number1) > stoi(number2)) {
        taskResult = task2InMemory(destination, number2, number1);
    } else {
        taskResult = task2InMemory(destination, number1, number2);
    }
    for (int i = 0; i < taskResult.size(); ++i) {
        cout << taskResult[i].getInText() << endl;
    }
...
void task3InMemory() {
    ...
    taskResult = task3InMemory(departure);
    for (int i = 0; i < taskResult.size(); ++i) {
        cout << taskResult[i].getInText() << endl;
    }
}
