...
void task1InText() {
    ...
    taskResult = task1InText(lastFragmentOfName);
...
void task2InText() {
    ...
    if (stoi(number1) > stoi(number2)) {
        taskResult = task2InText(destination, number2, number1);
    } else {
        taskResult = task2InText(destination, number1, number2);
    }
    for (int i = 0; i < taskResult.size(); ++i) {
        cout << taskResult[i].getInText() << endl;
    }
...
void task3InText() {
    ...
    taskResult = task3InText(departure);
    for (int i = 0; i < taskResult.size(); ++i) {
        cout << taskResult[i].getInText() << endl;
    }
}
